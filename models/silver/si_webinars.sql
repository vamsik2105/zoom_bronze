...
quality_checks AS (
    SELECT 
        *,
        -- Completeness checks
        CASE WHEN webinar_id IS NULL THEN 0 ELSE 1 END as webinar_id_complete,
        ...
        CASE WHEN registrants >= 0 THEN 1 ELSE 0 END as registrants_valid
...
scored_data AS (
    SELECT 
        *,
        ROUND(
            (webinar_id_complete + host_id_complete + start_time_complete + end_time_complete + time_logic_valid + registrants_valid) / 6.0, 2
